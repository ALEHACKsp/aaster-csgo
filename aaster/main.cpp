...
DWORD WINAPI OnDllAttach(LPVOID lpParameter)
{
	try
	{
		...
		L::ofsFile.open(C::GetWorkingPath().append(XorStr("aaster.log")), std::ios::out | std::ios::trunc);
		#endif
		 ...
		 */
		if (!CNetvarManager::Get().Setup(XorStr("netvars_aaster.txt")))
		...
		L::Print(XorStr("aaster successfully loaded"));
		L::PopConsoleColor();
		I::GameConsole->Clear();
		I::ConVar->ConsoleColorPrintf(Color(255, 50, 255, 255), XorStr("aaster successfully loaded.\nbuild date: %s / %s\n"), __DATE__, __TIME__);
	}
	catch (const std::exception& ex)
	{
		...
		MessageBox(nullptr, ex.what(), XorStr("aaster (error)"), MB_OK | MB_ICONERROR | MB_TOPMOST);
		#else
		// unload
