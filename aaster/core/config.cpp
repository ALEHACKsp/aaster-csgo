...
{
	// create directory "settings" in "%userprofile%\documents\aaster" if it incorrect or doesnt exists
...
bool C::Save(std::string_view szFileName)
{
	...
	if (fsFilePath.extension() != XorStr(".aas"))
		fsFilePath.replace_extension(XorStr(".aas"));
...
void C::Remove(const std::size_t nIndex)
{
	...
	if (!szFileName.compare(XorStr("default.aas")))
		return;
...
void C::Refresh()
{
    ...
    {
		if (it.path().filename().extension() == XorStr(".aas"))
		{
...
std::filesystem::path C::GetWorkingPath()
{
	...
	if (PWSTR pszPathToDocuments; SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Documents, 0UL, nullptr, &pszPathToDocuments)))
	{
		...
		fsWorkingPath.append(XorStr("aaster"));
